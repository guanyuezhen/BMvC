...
def regularization_loss(z_v1, z_v2, mask_v1_v2):
    z_v1 = F.normalize(z_v1, dim=1, p=2)
    z_v2 = F.normalize(z_v2, dim=1, p=2)
    s = torch.mm(z_v1, z_v2.t())
    pos_mask = mask_v1_v2
    neg_mask = 1 - mask_v1_v2
    pos_loss = torch.sum((pos_mask * (1 - s)).pow(2)) / (torch.sum(pos_mask))
    neg_loss = torch.sum((neg_mask * s).pow(2)) / (torch.sum(neg_mask))
    loss_con = pos_loss + neg_loss

    return loss_con


def knn_graph(data1, data2, k=15):
    num_samples = data1.shape[0]
    num_anchors = data2.shape[0]
    distances = torch.cdist(data1, data2) ** 2
    neighbors_idx = torch.argsort(distances, dim=1)[:, 0:k]
    d = distances[torch.arange(num_samples).unsqueeze(1), neighbors_idx]
    adjacency_matrix = torch.zeros((num_samples, num_anchors), dtype=data1.dtype, device=data1.device)
    eps = 1e-8
    d_k_minus_1 = d[:, -1]
    sum_d = torch.sum(d, dim=1)
    adjacency_matrix[torch.arange(num_samples).unsqueeze(1), neighbors_idx] = (
            (d_k_minus_1.unsqueeze(1) - d) /
            (k * d_k_minus_1.unsqueeze(1) - sum_d.unsqueeze(1) + eps)
    )
