...
def load_data(dataset):
    ...
    # sadj = knn_graph(s_features.cuda()).cpu().numpy()
    # sadj = sp.coo_matrix(sadj, dtype=np.float32)
    # fadj = knn_graph(features.cuda()).cpu().numpy()
    # fadj = sp.coo_matrix(fadj, dtype=np.float32)
...
if __name__ == "__main__":
    ...
    for i in range(len(datasets)):
        ...
        # df = pd.read_csv('./result/Human_Breast_Cancer/BMVC_idx.csv')
        # idx_max = df.iloc[:, 0].to_numpy()
        labels_cat = np.unique(labels)
        labels_cat_dict = {index: value for value, index in enumerate(labels_cat)}
        labels_int = [int(labels_cat_dict[x]) for x in labels]
        true = labels_int
        pred_1 = idx_max
        ...
        swapped_dict = {value: key for key, value in labels_cat_dict.items()}
        idx_max = [swapped_dict[x] for x in idx_max]
        # idx_max = np.array(idx_max)
        # adata.obs['idx'] = idx_max.astype(str)
        adata.obs['idx'] = idx_max
        # adata.obsm['emb'] = emb_max
        # adata.obsm['mean'] = mean_max
